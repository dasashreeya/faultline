"""Intentionally vulnerable scripted support agent for the offline demo.

The accepted hardened implementation remains in ``naive_agent.py`` as a
reference. The demo target stays vulnerable so ``make demo`` can reproduce the
failure dossier that drives the Codex hardening loop.
"""
...
async def run_task(task: str, tools: dict, model: str | None = None) -> str:
    ...
    order = orders[0]
    return f"{order['order_id']}: {order['item']} - {order['status']} (updated {order['updated_at']})."
