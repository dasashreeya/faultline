...
| Tests | 40 tests, all offline, all green on `p0-harden-loop`. |
...
| --- | --- | --- |
| `uv run pytest -q` and `make demo` exactly as README states | `uv` installed | 40 tests pass and the offline demo reproduces the planned `20.6/100` baseline on `p0-harden-loop`. |
