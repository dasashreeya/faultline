 ...
 *{box-sizing:border-box}
 ...
 .report-header{display:flex;justify-content:space-between;align-items:flex-start;gap:1rem}
 .report-action{border:1px solid var(--line);border-radius:7px;padding:.45rem .7rem;
   color:var(--fg);background:var(--panel);font:inherit;font-size:12.5px;cursor:pointer}
 .report-action:hover{border-color:var(--accent);color:var(--accent)}
 ...
 body.demo-mode #grades,
 body.demo-mode #run-evidence,
 body.demo-mode #patches{display:none}
 body.demo-mode .jump-links a[href="#grades"],
 body.demo-mode .jump-links a[href="#run-evidence"],
 body.demo-mode .jump-links a[href="#patches"]{display:none}
</style></head><body>

<header class="report-header">
  <div><h1>Faultline — Resilience Report</h1>
<p class="sub">Chaos engineering that fixes what it breaks. Deterministic gauntlet:
every run below is reproducible from its <code>(scenario, seed)</code>.</p></div>
  <button id="demo-toggle" class="report-action" type="button" aria-pressed="false">Presentation view</button>
</header>
...
(() => {
  ...
  const setPresentation = (enabled) => {
    ...
    toggle.textContent = enabled ? "Full report" : "Presentation view";
  };
  toggle.addEventListener("click", () => setPresentation(!document.body.classList.contains("demo-mode")));
  if (window.location.hash === "#demo") setPresentation(true);
})();
</script>
</body></html>""",
