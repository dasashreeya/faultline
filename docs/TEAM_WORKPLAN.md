...
- `uv run pytest -q`: 130 offline tests pass on merged local `main` plus the frontier backend, static report chart, and dense-history chart coverage.
...
- interactive resilience frontier charts (the deterministic JSON backend and static report presentation are implemented; user demo narration remains);
